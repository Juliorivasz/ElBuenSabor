...
import { ModificarInsumoDto } from "../../models/dto/ModificarInsumoDto"

interface InsumoFormProps {
  insumo?: InsumoAbmDto
  unidadesDeMedida: UnidadDeMedidaDto[]
  rubrosInsumo: RubroInsumoAbmDto[]
  onSubmit: (insumo: NuevoInsumoDto | ModificarInsumoDto) => void
  onCancel: () => void
  loading: boolean
  isEditing: boolean
}
...
export const InsumoForm: FC<InsumoFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!isEditing && formData.stockActual < 0) {
    ...
    if (formData.stockMaximo <= formData.stockMinimo) {
      newErrors.stockMaximo = "El stock máximo debe ser mayor al stock mínimo"
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      const modificarInsumo = new ModificarInsumoDto(
        formData.nombre,
  ...
  const title = isEditing ? "Editar Insumo" : "Nuevo Insumo"
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-black">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* Stock Actual - Solo para creación */}
              {!isEditing && (
                <div>
