...
import { esRubroPadre, isRubroActivo } from "../../models/dto/RubroInsumoDto"
  ...
  const renderRubroRow = (rubro: RubroInsumoDto, nivel = 0, esSubrubro = false): React.ReactNode => {
    ...
    return (
      <>
        ...
        <tr
          key={`rubro-${rubro.getIdRubroInsumo()}`}
        ...
        >
          ...
          <td className="px-6 py-4 whitespace-nowrap">
            ...
            >
              <span
                className={`w-2 h-2 rounded-full mr-2 ${isRubroActivo(rubro) ? "bg-green-400" : "bg-red-400"}`}
              ></span>
              {isRubroActivo(rubro) ? "Activo" : "Inactivo"}
            </span>
          </td>
            ...
            <div className="flex items-center justify-end space-x-1">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  isRubroActivo(rubro) ? "bg-green-500 focus:ring-green-500" : "bg-red-400 focus:ring-red-400"
                }`}
                title={isRubroActivo(rubro) ? "Desactivar" : "Activar"}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    isRubroActivo(rubro) ? "translate-x-6" : "translate-x-1"
                  }`}
                />
              </button>
              <button
