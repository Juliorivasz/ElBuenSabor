...
import { CartStore } from "./types/cart"; // Importa las interfaces que definimos antes
...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  // Recibe el producto (`articulo`) que se va a agregar y, opcionalmente, la URL de la imagen del producto.
  addItem: (articulo, imagenUrl) => {
    set((state) => {
      ...
      if (existingItem) {
        ...
        return { items: [...state.items, { articulo, quantity: 1, imagenUrl }] };
        // Agregamos el nuevo producto al array `items`.
        // La cantidad inicial es 1, y guardamos también la URL de la imagen (si la tenemos).
      }
    });
  },
  ...
  getTotalPrice: () => {
    return get().items.reduce((total, item) => total + item.articulo.getPrecioVenta() * item.quantity, 0);
    // Similar a `getTotalItems`, pero en lugar de sumar solo la cantidad, sumamos el precio de cada producto
