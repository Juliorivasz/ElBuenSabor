...
export const useCategoriasStore = create<CategoriasState>((set, get) => ({
  ...
  crearCategoria: async (categoria: NuevaCategoriaDto) => {
    set({ loading: true, error: null });
    try {
  ...
  actualizarCategoria: async (idCategoria: number, categoria: NuevaCategoriaDto) => {
    set({ loading: true, error: null });
    try {
  ...
  toggleEstadoCategoria: async (idCategoria: number) => {
    ...
    if (subcategorias.length > 0) {
      if (categoria.isActiva()) {
        ...
        mensajeConfirmacion += `\n\nEsto también desactivará ${subcategorias.length} subcategoría${
          subcategorias.length > 1 ? "s" : ""
        } asociada${subcategorias.length > 1 ? "s" : ""}:`;
      } else {
        // Activando categoría padre
        mensajeConfirmacion += `\n\nEsto también activará ${subcategorias.length} subcategoría${
          subcategorias.length > 1 ? "s" : ""
        } asociada${subcategorias.length > 1 ? "s" : ""}:`;
      }
