...
export const Cart = () => {
  ...
  const getEstimatedCookingTime = () => {
    ...
    const maxCookingTime = Math.max(
      ...items.map((item) => {
        // Para productos, obtener su tiempo de cocina
        ...
        // Para promociones, usar el tiempo de cocina más largo de sus artículos
        if (item.type === "promotion" && item.promocion) {
          const tiemposPromo = item.promocion.getDetalles().map((detalle) => {
            // Asumimos un tiempo promedio para artículos de promoción
            console.log(detalle)
            return 10 // Tiempo estimado por defecto para artículos de promoción
          })
          return Math.max(...tiemposPromo, 0)
        }
        return 0
      }),
    )
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 bg-white/80 backdrop-blur-sm rounded-full px-4 sm:px-6 py-3 shadow-lg border border-orange-100 overflow-x-auto">
              {[
                ...
                <div key={step.label} className="flex items-center flex-shrink-0">
                  <div
                    className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full transition-all duration-300 ${step.active ? "bg-gradient-to-br from-orange-500 to-red-500 text-white shadow-lg" : "bg-gray-200 text-gray-400"}`}
                  >
                    <step.icon sx={{ fontSize: { xs: 16, sm: 20 } }} />
                  </div>
                  <span
                    className={`ml-1 sm:ml-2 text-xs sm:text-sm font-medium ${step.active ? "text-orange-600" : "text-gray-400"}`}
                  >
                    {step.label}
                  </span>
                ...
                <AnimatePresence>
                  <div className="space-y-4">
                    {items.map((item, index) => {
                      const itemKey =
                        item.type === "product"
                          ? `product-${item.articulo?.getIdArticulo()}`
                          : `promotion-${item.promocion?.getIdPromocion()}`
